...
export default function SoloPlayerBoard({
  ...
  const handleButtonClick = (event) => {
    ...
    if (clickedButton.classList.contains('new-game-btn')) {
      if (typeof onSetup === 'function') {
        onSetup();
      } else {
        startNewGame();
      }
    } else if (clickedButton.classList.contains('restart-btn')) {
      restartGame();
    }
  };
  ...
  const handleFlipCard = (card, index) => () => {
    ...
    if (nextClicked.length === 2) {
      setIsResolving(true);
      // count a move when two cards have been flipped
  ...
  return (
    <div className='solo-player-board'>
      <header>
        <div className='logo'>
        ...
        <div className='btns'>
          <button className='restart-btn' onClick={handleButtonClick}>Restart</button>
          <button className='new-game-btn' onClick={handleButtonClick}>New Game</button>
        </div>
      </header>
